...
try {
    ...
    $stmt = $pdo->query("SHOW COLUMNS FROM `admob_ads` LIKE 'ad_type'");
    $ad_type_exists = $stmt->rowCount() > 0;

    $stmt = $pdo->query("SHOW COLUMNS FROM `admob_ads` LIKE 'description'");
    $description_exists = $stmt->rowCount() > 0;

    if (!$ad_type_exists) {
        $pdo->exec("ALTER TABLE admob_ads ADD COLUMN ad_type VARCHAR(255) NOT NULL DEFAULT 'banner' AFTER ad_unit_id");
    }

    if (!$description_exists) {
        $pdo->exec("ALTER TABLE admob_ads ADD COLUMN description TEXT AFTER is_enabled");
    }
} catch (PDOException $e) {
    // Log the error or handle it gracefully
    // For example, you can log to a file: error_log("Migration failed: " . $e->getMessage());
}
